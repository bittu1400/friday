...
| G7 | IS-1..IS-20 all blocked, asserted on the executor. **The egress half of this row was withdrawn 2026-08-30** (D13/D15) and is **REINSTATED 2026-09-02 on new evidence** — see the two rows below. |
| 2026-09-03 | **The controls this file names were not all watched by a test, and now the load-bearing five are.** A mutation audit (ADR-116) injected 85 defects one at a time: **three of T4's confirm gates could have their branch deleted from `turn.py` with all 581 tests passing** — `system_wifi{off}` dispatched and dropped the network, `hypr_window{close}` closed the window, neither asked — and **T2's ban list stopped being consulted at dispatch** (`assert_not_banned(argv)` → `pass`) with the adversarial and injection suites still green, because `ban.py` and `executor.py` were each thoroughly unit-tested and **nothing crossed between them**. Also: the `"rm"` entry in `BANNED_BINARIES` could be removed silently, since the only argv testing it (`["rm","-rf","/"]`) is *also* caught by the `"rm -"` substring rule; FR-32's minimal explicit env was read by no test; and `SpeakerVerifier.verify()` was called by no test in the repository. **None was a defect — the code was correct throughout.** All five are now pinned (ADR-117): `tests/test_confirm_arming.py`, `tests/test_executor.py::test_banned_argv_is_denied_at_dispatch`, an argv only the binary rule rejects in `tests/test_action_surface.py`, `test_subprocess_gets_the_minimal_explicit_env_only`, and `test_verify_accepts_the_owner_and_rejects_an_impostor`. Each was verified by applying the mutation and watching the suite turn red. **Process consequence for this file: a control listed here is only as real as the test that fails when it is removed** — and that is now line six of the definition of done. |
| 2026-09-03 | **A deployment control was being verified against itself.** `tests/test_service_unit.py` parses `deploy/systemd/friday.service` from disk, and the installed unit is a **symlink to that file** — so it always matched, while `systemctl show` reported `Type=simple`, `WatchdogUSec=0`, `NeedDaemonReload=yes` and the committed watchdog had never once fired (M16). Control added: `friday/selftest.py::check_unit_deployed` asks `systemctl --user show` for `LoadState`, `NeedDaemonReload` and the four directives this project has been bitten by — `Type`, `WatchdogUSec`, `PrivateTmp` (D30) and `KillMode` (D29). Drift or a pending reload is a FAIL; no user bus is a WARN. Six FAIL/WARN paths tested. **Editing a unit is not deploying it** (OQ-66 → ADR-117). |
| 2026-09-02 | **The egress claim now has a check that can observe an egress event, and it immediately found two.** `just test-egress` (ADR-110) guards `socket.getaddrinfo` and `socket.socket.connect`, exercises the real STT load path, and inspects the **live daemon's** sockets. Its FAIL path is demonstrated, not asserted: removing `local_files_only=True` makes it name `huggingface.co`. The two previous versions could not see egress at all — v1 inspected *listening* sockets (D15), v2 asserted `urlparse()` on config constants. **Every "egress proof" in this repo written before ADR-110 traces to one of those two and must not be cited.** |
| 2026-09-02 | **`PrivateTmp=yes` removed from `friday.service` — a deliberate reduction in sandboxing, recorded here rather than made quietly (ADR-115, D30).** It was breaking every GUI launch: Chromium keeps its singleton socket in `/tmp` and only a symlink to it under `$HOME`, so a Friday-launched Brave saw the shared lock, could not reach the socket in its private empty `/tmp`, and exited 0 in ~50 ms with no window — announced as a successful launch. It also hid `/tmp/.X11-unix`, so ADR-043's `DISPLAY=:0` could never have reached XWayland. **Assessment: the directive separated no trust domains.** The daemon runs as the user, and the apps it launches are the user's own apps in the user's own session; a private `/tmp` isolated the user from themselves. Every control this threat model actually relies on is untouched and none reads `/tmp`: the closed enum (T2), code-built argv (FR-32), the ban list, `plan.gbnf`/`final.gbnf` (T1), the three confirm tiers, and the panic switch. `ProtectSystem=strict`, `NoNewPrivileges=yes` and `ReadWritePaths=` are kept. `tests/test_service_unit.py` fails if it is set again, so restoring it is a decision someone has to make on purpose. **Residual risk accepted:** a hostile process running as this same user could now plant or read files in the shared `/tmp` that a Friday-launched app touches — but such a process already has the user's full authority, including write access to `~/.local/share/applications`, which is a strictly stronger foothold (F5). |
| 2026-09-02 | **D27 — `import onnxruntime` transmits to Microsoft, and did so for the life of the project.** The live daemon held established HTTPS connections to `*.events.data.microsoft.com` (certificate read directly off the endpoint). It is on **import**, on Linux, with no inference: a process whose entire body is `import onnxruntime` opens the socket within 15–45 s, while a bare interpreter and `import numpy` stay clean over the same window. **Five components route through ORT** — Silero VAD, openWakeWord, Kokoro TTS, CAM++/sherpa-onnx — so every daemon start did it. `onnxruntime.disable_telemetry_events()` does **not** stop it; only `ORT_DISABLE_TELEMETRY=1`, set before the library loads. Control: `friday/__init__.py` sets it via `os.environ.setdefault`, `deploy/systemd/friday.service` sets `Environment=ORT_DISABLE_TELEMETRY=1`, and `tests/test_egress.py` guards both the variable and the live process. Verified clean over 60 s after the fix (**ADR-112**, FR-133). **The payload was not inspected**; ORT documents its telemetry as build/EP/model metadata rather than user data, and nothing observed contradicts that, but this repo's first invariant does not permit assuming. |
| 2026-09-02 | **Process lesson for this file: rule 7 vets a new dependency's FOOTPRINT — it has never asked what the dependency TALKS TO.** D13 and D27 both entered through that gap, in two different libraries, and neither was visible to any test for months. A dependency drill should now include one `import <pkg>; sleep 60` run watched with `ss -tnp`. |
