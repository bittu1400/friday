...
| FR-130 | Systemd watchdog integration via `$NOTIFY_SOCKET` / `$WATCHDOG_USEC` with `Type=notify` and periodic heartbeat task | event loop lockup prevents ping, causing systemd watchdog timeout restart | **BUILT Phase 2** (ADR-109) |
